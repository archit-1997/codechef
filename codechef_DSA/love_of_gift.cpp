...
// See how you can assign id's while sorting so that you can easily
// differentiate between two different kinds of elements in the same list
...
// Now defining the function f(x) which gives us the number of people that reach
// byteland when the inital nubmer of people starting from byteland is x
...
int main() {
  ...
  while (t--) {
    ll X;
    cin >> X; // The distance of byteland from chefs town
    ll b;
    cin >> b; // Number of dishes
    ...
    if (C == 0) {
      ...
      // now push the last element that indicates that we have finally reached
      // the answer -20 is the id for byteland and -50 is just a random value
      v.pb({X, {-20, -50}});
      sort(v.begin(), v.end());
